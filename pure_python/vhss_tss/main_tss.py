...
def main_tss():
    ...
    for i in range(1, nr_clients + 1):  # Generation clients
      ...
      if (i != nr_clients):
          ...
          R_i = math.ceil(R_is / (q_field_for_secret_sharing - 1)) * (q_field_for_secret_sharing - 1) - R_is
          #print("R_is: {}".format(R_is))
          #print("ceil: {}".format(math.ceil(R_is / (q_field_for_secret_sharing - 1))))
    ...
    for c in clients:
      print("client_id {}".format(c.get_id()))
    ...
    result_verify = tss.verify(nr_clients, H_is, final_proof_test, final_eval, g, N)
